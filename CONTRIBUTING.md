# Contributing to mdi-ts

mdi-ts has been created to fulfill the National Bank of Belgium's front-end development needs, but is now an open source project.

As such, third party contributions are more than welcome! Here you'll find all the information you need to get started.

We would love for you to contribute to mdi-ts and make it better and better.
As a contributor, here are the guidelines we would like you to follow.

## Code of Conduct

Help us keep mdi-ts open and inclusive. Please read and follow our [Code of Conduct](/CODE_OF_CONDUCT.md).

## Got a question or problem?

Ask questions on [Stack Overflow](https://stackoverflow.com/questions/tagged/nbbMdiTS) where the questions should be tagged with tag `nbbMdiTS`
...
Take a look at the project's open [issues](https://github.com/NationalBankBelgium/mdi-ts/issues) and [milestones](https://github.com/NationalBankBelgium/mdi-ts/issues/milestones).
...
- version of mdi-ts & styling/linting tool used
...
You can file new issues by filling out our [new issue form](https://github.com/NationalBankBelgium/mdi-ts/issues/new).

### Forking

mdi-ts development is done using a forking model with Pull Requests (PRs), so the very first thing you should do is create your fork: https://help.github.com/articles/fork-a-repo/
...
Once you're found what you want to contribute to mdi-ts, then:

- Create a feature branch in your fork: `git checkout -b my-new-branch master`
- Configure the upstream
  - `git remote add upstream https://github.com/NationalBankBelgium/mdi-ts.git`
...
Once done, you may submit a new Pull Request (PR): https://github.com/NationalBankBelgium/mdi-ts/pull/new/master
...
Here's a summary of the workflow we recommend to contribute to mdi-ts along with the commands to use. Note that this only uses a subset of what git flow has to offer.
...
- fork: through GitHub
- clone: `git clone https://github.com/<username>/mdi-ts`
- add upstream: `git remote add upstream https://github.com/NationalBankBelgium/mdi-ts`
  - that way you can fetch new commits that have been integrated into mdi-ts's main repository
...
- add the other developer's fork as a remote: `git remote add <username> https://github.com/<username>/mdi-ts`
...
- make changes
- push the changes
- sync your local repo with the main repository of mdi-ts
- create PRs against the main repository of mdi-ts (or let the other developer do it)

In any case, after PRs are merged into mdi-ts, you can normally forget about the feature branches and delete those to keep your fork clean
...
If you want to build mdi-ts from source, you need to...

- install NodeJS (18.0.0+) and npm (10.0.0+)
- clone this git repository: `git clone https://github.com/NationalBankBelgium/mdi-ts.git`
- run `npm run install` from the root directory of mdi-ts

### Executing mdi-ts's main build script

Anytime you make modifications to the mdi-ts library, you'll need to

- execute `npm run build` from the root directory of mdi-ts

That command will then build mdi-ts using the [build script](/build.sh).

### Hacking the mdi-ts library

If you want to modify the mdi-ts library:

- go to the package folder. `cd packages/<name>`
- install its dependencies: `npm install`
- make your changes
- execute `npm run build` at mdi-ts's root
...
You can execute the tests for the mdi-ts library using the following command at the root: `npm test`
